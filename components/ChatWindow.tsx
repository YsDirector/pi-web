...
import { parseBatchAskEnvelope } from "@/lib/batch-ask";
import { BatchAskDialog } from "./BatchAskDialog";
...
/**
 * 折叠态弹窗条目的摘要行（上游 0.9.0）：select 取第一个选项，confirm 取 message 首行。
 */
...
function ExtensionDialog({
  ...
  // 超时倒计时（fork 特性）：0.8.10+ 服务器把 timeout 换算成 expiresAt；剩余 <10s 高亮，到点自动取消
  const remainingMs = request.expiresAt === undefined ? null : Math.max(0, request.expiresAt - now);
  const remainingSeconds = remainingMs === null ? null : Math.ceil(remainingMs / 1000);
  const urgent = remainingMs !== null && remainingMs < 10000;
  ...
  // 到点自动取消（fork 客户端兜底；服务器也会通过 extension_ui_closed 关闭过期请求）
  useEffect(() => {
      ...
      >
        <div style={{ flexShrink: 0, display: "flex", alignItems: "flex-start", gap: 8, padding: "12px 14px", borderBottom: "1px solid var(--border)" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: "var(--text)", fontSize: 14, fontWeight: 650, lineHeight: 1.4 }}>{titleHead}</div>
